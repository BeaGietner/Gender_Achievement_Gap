...
gender_gap_data_JC <- bind_rows(
  tibble(Subgroup = "High Income", gender_gap_high_income_W2),
  tibble(Subgroup = "Low Income", gender_gap_low_income_W2),
  tibble(Subgroup = "High Education PCG", gender_gap_high_educ_PCG_W2),
  tibble(Subgroup = "Low Education PCG", gender_gap_low_educ_PCG_W2),
  tibble(Subgroup = "High Education SCG", gender_gap_high_educ_SCG_W2),
  tibble(Subgroup = "Low Education SCG", gender_gap_low_educ_SCG_W2)
)
...
calculate_gender_gap_LC <- function(means) {
  ...
  data.frame(
    Maths_Gap_LC = male_means_LC$LC_Maths_Points - female_means_LC$LC_Maths_Points,
    English_Gap_LC = male_means_LC$LC_English_Points - female_means_LC$LC_English_Points
  )
}
...
# Combine gender gap data into a single data frame
gender_gap_data_LC <- bind_rows(
  tibble(Subgroup = "High Income", gender_gap_high_income_W3),
  tibble(Subgroup = "Low Income", gender_gap_low_income_W3),
  tibble(Subgroup = "High Education PCG", gender_gap_high_educ_PCG_W3),
  tibble(Subgroup = "Low Education PCG", gender_gap_low_educ_PCG_W3),
  tibble(Subgroup = "High Education SCG", gender_gap_high_educ_SCG_W3),
  tibble(Subgroup = "Low Education SCG", gender_gap_low_educ_SCG_W3)
)

# Invert the gap values for each subject
gender_gap_data_inverted_LC <- gender_gap_data_LC %>%
  pivot_longer(cols = c(Maths_Gap_LC, English_Gap_LC), names_to = "Subject", values_to = "Gap") %>%
  mutate(Gap = -Gap)

# Check the prepared data
head(gender_gap_data_inverted_LC)

# Plotting
ggplot(gender_gap_data_inverted_LC, aes(x = Subgroup, y = Gap, color = Subject, group = Subject)) +
  geom_point(size = 3, position = position_dodge(width = 0.0)) +  
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray") +  
