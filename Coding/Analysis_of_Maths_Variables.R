...
math_cont_summary <- ml_data_complete %>%
...
math_wave2_summary <- ml_data_complete %>%
  filter(!is.na(resilience)) %>%
  select(ID, resilience) %>%
...
sdq_wave2_summary <- ml_data_complete %>%
  filter(!is.na(resilience)) %>%
  select(ID, resilience) %>%
...
math_wave3_summary <- ml_data_complete %>%
  filter(!is.na(resilience)) %>%
  select(ID, resilience) %>%
...
sdq_wave3_summary <- ml_data_complete %>%
  filter(!is.na(resilience)) %>%
  select(ID, resilience) %>%
  inner_join(select(Merged_Child, ID, 
